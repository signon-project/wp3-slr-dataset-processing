...
def _split_on_participant(df, rand_seed):
        unique_participants = np.unique(df.participant.values)
        train_participants, eval_participants = train_test_split(unique_participants, test_size=0.2, random_state=rand_seed)
        ...
        assert len(set(train_df.participant.unique()) & set(val_df.participant.unique()) & set(test_df.participant.unique())) == 0
...
def encode_labels(df):
    ...
    dfc['Participant'] = part_encoder.fit_transform(dfc.participant.values)
    ...
    #Drop "gloss" and "participant" columns:
    dfc.drop(['gloss', 'participant'], axis=1, inplace=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv_in', type=str, help='Input .csv file from prep_data.py step.')
    ...
    prep_df = pd.read_csv(args.csv_in)

    #2. Verify this is in the right form for cleaning/processing:
    #   Form: ['video_name', 'fps', 'start_ms', 'end_ms', 'gloss', 'participant']
    col_names = list(prep_df)
    if not col_names == ['video_name', 'fps', 'start_ms', 'end_ms', 'gloss', 'participant']:
        raise Exception("CSV not in correct form. Check column values.")

    #3. Perform stratified split:
    #   Form should now be: ['video_name', 'fps', 'start_ms', 'end_ms', 'gloss', 'Subset']
    _tmp, glosses = stratified_split(prep_df)
    ...
    #4. Encode labels: 
    #   Form will now be: ['video_name', 'fps', 'start_ms', 'end_ms', 'Subset', 'Participant', 'Label']
    out_df = encode_labels(_tmp)
