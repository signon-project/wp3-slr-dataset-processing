...
def main(args):
    ...
    with torch.no_grad():
        ...
        cap.set(cv2.CAP_PROP_POS_FRAMES, args.start_seconds * fps)
        current_frame_index = args.start_seconds * fps
        while cap.isOpened():
            ...
            model_input = frame.unsqueeze(0)

            model_output = model(model_input).item()
            model_prediction = model_output > 0.5
            ...
            if not previous_prediction and output_prediction:
                ...
                current_segment['end'] = (current_frame_index - 1) // fps
